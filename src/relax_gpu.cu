...
//   dm/dt = -γ'αμ₀ m×(m×H)   (no precession)
//   m_new = m + dt * dm/dt,   then normalise
// ===========================================================================
__global__ static void damping_euler_kernel(
    double* __restrict__ m,        // [3N] updated in-place
    const double* __restrict__ H,  // [3N] effective field
...
// ===========================================================================
// max-torque reduction: writes max |m×H|² (atomicMax) to d_max[0].
// Caller must set d_max[0] = 0 before launch.
// ===========================================================================
__global__ static void max_torque_kernel(
    double* __restrict__       d_max,  // [1] output
...
// ===========================================================================
// Per-cell energy: e[i] = -mu0/2 * Ms * m[i]·H[i] * dV  (exchange + demag)
// We store just m·H (energy density / (mu0*Ms*dV/2))
// For total energy, the caller sums and scales.
// Actually, let's store m·H (dimensionless dot product), and scale later.
// ===========================================================================
__global__ static void mdot_H_kernel(
    double* __restrict__       e_out,  // [N] m·H per cell
    const double* __restrict__ m,
    const double* __restrict__ H,
    int N)
{
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= N) return;
    e_out[idx] = m[0*N+idx]*H[0*N+idx] + m[1*N+idx]*H[1*N+idx] + m[2*N+idx]*H[2*N+idx];
}
...
// ===========================================================================
__global__ static void copy3N_kernel(double* dst, const double* src, int N3) {
...
// ===========================================================================
static void relax_alloc(size_t N, double*& d_m, double*& d_H,
                         double*& d_max, void*& stream,
                         double** d_trial = nullptr,
    ...
    if (d_trial)  CUDA_CHECK(cudaMalloc(d_trial,  3 * N * sizeof(double)));
...
static void relax_free(double* d_m, double* d_H, double* d_max, void* stream,
                        double* d_trial = nullptr, double* d_energy = nullptr) {
...
static void pack3N(const VectorField3D& m, size_t N, std::vector<double>& buf) {
    buf.resize(3 * N);
...
static void unpack3N(const std::vector<double>& buf, size_t N, VectorField3D& m) {
    for (Index i = 0; i < static_cast<Index>(N); ++i) {
        m[i].x = buf[i];
        m[i].y = buf[N   + i];
        m[i].z = buf[2*N + i];
...
void RelaxGPU::upload(const VectorField3D& m) {
    ...
    pack3N(m, N_, buf);
    CUDA_CHECK(cudaMemcpy(d_m_, buf.data(), 3*N_*sizeof(double), cudaMemcpyHostToDevice));
}

void RelaxGPU::download(VectorField3D& m) const {
    std::vector<double> buf(3 * N_);
    CUDA_CHECK(cudaMemcpy(buf.data(), d_m_, 3*N_*sizeof(double), cudaMemcpyDeviceToHost));
    unpack3N(buf, N_, m);
}
...
void MinimizeGPU::compute_H_eff_for(double* d_m_src,
                                     const Material& mat,
                                     IDemagGPU& demag,
                                     ExchangeFieldGPU& exch,
                                     ZeemanFieldGPU& zeeman,
...
double MinimizeGPU::compute_energy(const Material& mat,
    ...
    // E = -μ₀/2 * Ms * sum(m·H) * dV   (standard LLG energy sign)
